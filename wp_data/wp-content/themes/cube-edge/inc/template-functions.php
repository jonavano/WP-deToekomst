 ...
 * @return array
 */
function cube_edge_body_classes( $classes ) {
...
add_filter( 'body_class', 'cube_edge_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function cube_edge_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'cube_edge_pingback_header' );

/**
 * cube_edge Excerpt Length
 *
 * @since cube_edge 1.0.0
 *
 * @param null
 * @return void
 */

if ( ! function_exists( 'cube_edge_excerpt_length' ) ) :
   ...
   */
  function cube_edge_excerpt_length( $length ) {
...
add_filter( 'excerpt_length', 'cube_edge_excerpt_length', 999 );

if ( ! function_exists( 'cube_edge_get_post_categories' ) ) {
	/**
	* Get post categories by by term id
	* 
	* @return array
	*/
	function cube_edge_get_post_categories(){
